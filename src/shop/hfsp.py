...
class Hfsp(Schedule):
    ...
    def decode_permutation_nw_lw(self, code):
        self.clear()
    ...
    def decode_permutation_classic(self, code):
        self.clear()
        copy_code = deepcopy(code)
        mac, j = [[None for _ in range(job.nop)] for job in self.job.values()], 0
        while self.any_task_not_done():
            for i in copy_code:
                ...
                self.job[i].task[j].end = end[choice]
                if self.job[i].task[j].resumable is not None:
                    res1, res2 = self.constrain_timetable(i, j, k, p, None)
                    if res1 is False:
                        continue
    ...
    def decode_permutation(self, code):
        if self.job[0].task[0].limited_wait is not None:
            return self.decode_permutation_nw_lw(code)
        return self.decode_permutation_classic(code)

    def decode_permutation_timetable(self, code):
        return self.decode_permutation_nw_lw(code)

    def decode_permutation_hfsp(self, code, mac):
        self.clear()
