...
def GaTemplate(save, instance, ga, n_exp=10, ga2=None):
    ...
    for exp in range(1, n_exp + 1):
        ...
        ga.best[0].trans_direction()
        ga.best[0].save_gantt_chart_to_csv("./%s/%s/GanttChartReal/%s.csv" % (save, instance, exp))
        ...
        if ga2 is not None:
            ...
            ga2.best[0].trans_direction()
            ga2.best[0].save_gantt_chart_to_csv("./%s/%s/GanttChartReal2/%s.csv" % (save, instance, exp))
